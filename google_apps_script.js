...
var productsCatalog = [
  { id: "ready_kolkata_daliya", name: "Kolkata Daliya", category: "Bikaneri (Bina Sika) Sattu", prices: [190, 375, 750, 940] },
  { id: "ready_mumbai_daliya", name: "Mumbai Daliya", category: "Bikaneri (Bina Sika) Sattu", prices: [190, 375, 750, 940] },
  { id: "ready_gehu", name: "Gehu", category: "Bikaneri (Bina Sika) Sattu", prices: [175, 350, 700, 875] },
  { id: "ready_rice", name: "Rice", category: "Bikaneri (Bina Sika) Sattu", prices: [175, 350, 700, 875] },
...
function doPost(e) {
  try {
    ...
    productsCatalog.forEach(function(prod) {
      var prefix = (prod.category === "Bikaneri (Bina Sika) Sattu" ? "Bina Sika" : "Sika hua") + " - " + prod.name + " ";
    ...
    // Generate dynamic Web App redirect hyperlink formula for confirming cash payment
    var webAppUrl = ScriptApp.getService().getUrl();
    var formula = '=HYPERLINK("' + webAppUrl + '?action=confirm&row=" & ROW(), "🟢 Send Confirmation")';
    ...
    // Send the email using Gmail Service only if a valid email is provided
...
function beautifySheet(sheet) {
  ...
  // Neutral dark gray for metadata columns (Columns 1 to 13)
  var metaHeaderRange = sheet.getRange(1, 1, 1, Math.min(13, lastColumn));
  ...
  // Amber/Orange for Ready Sattu columns (Columns 14 to 29: 4 products * 4 sizes)
  if (lastColumn >= 14) {
    var readyHeaderRange = sheet.getRange(1, 14, 1, Math.min(16, lastColumn - 13));
  ...
  // Emerald Green for Sika Sattu columns (Columns 30 to 41: 3 products * 4 sizes)
  if (lastColumn >= 30) {
    var sikaHeaderRange = sheet.getRange(1, 30, 1, Math.min(12, lastColumn - 29));
  ...
  if (lastRow > 1) {
    ...
    sheet.getRange(2, 12, lastRow - 1, 1).setHorizontalAlignment("center").setFontColor("#1d4ed8").setFontWeight("bold"); // Send Confirmation Link
    sheet.getRange(2, 13, lastRow - 1, 1).setHorizontalAlignment("center").setFontColor("#16a34a").setFontWeight("bold"); // Confirmation Status
    ...
    if (lastColumn >= 14) {
      var qtyColsRange = sheet.getRange(2, 14, lastRow - 1, lastColumn - 13);
...
function generateConsolidatedReport(ss, dataSheet) {
  ...
  var rowValues = [];
  var startCol = 14; // Column N in Sheet1
...
// Handle GET request (Redirects to WhatsApp and marks order as confirmed in Sheets)
function doGet(e) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    if (action === "confirm" && row) {
      // Column 13 (Column M) is "Confirmation Status"
      sheet.getRange(row, 13).setValue("Sent (" + new Date().toLocaleString() + ")");
      sheet.getRange(row, 13).setFontColor("#16a34a").setFontWeight("bold");
      
      // Fetch details from spreadsheet to build pre-filled text
      var orderId = sheet.getRange(row, 2).getValue();
      var custName = sheet.getRange(row, 3).getValue();
      var custMobile = sheet.getRange(row, 4).getValue();
      var grandTotal = sheet.getRange(row, 10).getValue();
      var paymentMethod = sheet.getRange(row, 7).getValue(); // Column G (7)
      
      var payText = (paymentMethod === "UPI") ? "UPI payment" : "Cash payment";
      var text = "*Kendriya Mahila Samiti - Payment Confirmed*\n" +
                 "Dear " + custName + ",\n" +
                 "We have received your " + payText + " of ₹" + grandTotal + " for Order ID " + orderId + ". Your order is now confirmed! Thank you.";
      
      var waPhone = custMobile.toString().replace(/\D/g, '');
      if (waPhone.length === 10) waPhone = "91" + waPhone;
      
      var waUrl = "https://wa.me/" + waPhone + "?text=" + encodeURIComponent(text);
      
      var html = "<html>" +
                 "<body style='font-family: Arial; text-align: center; padding-top: 100px; color: #374151;'>" +
                 "<h2>Opening WhatsApp...</h2>" +
                 "<p>Marking confirmation status as Sent in Google Sheets.</p>" +
                 "<p>If WhatsApp doesn't open automatically, <a href='" + waUrl + "' target='_top'>click here</a>.</p>" +
                 "<script>" +
                 "  window.open('" + waUrl + "', '_top');" +
                 "</script>" +
                 "</body></html>";
                 
      return HtmlService.createHtmlOutput(html).setTitle("Redirecting to WhatsApp...");
    }
    
    // Fallback logic for reading orders (kept for backwards compatibility)
