...
function calculateTotals() {
  ...
  if (upiLink) {
    if (grandTotal > 0) {
      const upiIdStr = "sattuorder@upi"; // Global UPI ID
      const upiUrl = `upi://pay?pa=${upiIdStr}&pn=Kendriya%20Mahila%20Samiti&am=${grandTotal.toFixed(2)}&cu=INR&tn=KMS%20Sattu%20Order`;
      upiLink.href = upiUrl;
...
function submitOrder() {
  ...
  processSubmitOrder(orderId, kshetra, custName, custMobile, "UPI", upiId, activeItems, totalQty, grandTotal);
}
  ...
  const billHTML = `
    <div class="bill-container">
      ...
      <p class="bill-details">
        ...
        <strong>UPI Transaction ID (UTR):</strong> ${upiId}
      </p>
